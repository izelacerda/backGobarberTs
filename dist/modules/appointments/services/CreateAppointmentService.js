...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if ((0, _dateFns.getHours)(appointmentDate) < 8 || (0, _dateFns.getHours)(appointmentDate) > 17) {
      throw new _AppError.default('You can only create an appointment between 8am and 5pm.');
    }

    const findAppoinmentInSameDate = await this.appointmentsRepository.findByDate(appointmentDate, provider_id);

    if (findAppoinmentInSameDate) {
      throw new _AppError.default('Appointment is Already exist!');
    }
    ...
    const dateFormatted = (0, _dateFns.format)(appointmentDate, "dd/MM/yyyy 'as' HH:mm'h'");
